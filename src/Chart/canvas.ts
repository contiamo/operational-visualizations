...
class ChartCanvas implements Canvas {
  ...
  private renderLegend(options: { position: LegendPosition; float: LegendFloat }) {
  ...
  private insertLegend(legendNode: Element) {
    const ref = this.drawingContainer.node();
    ref.parentNode.insertBefore(legendNode, ref);
  }

  private appendLegend(legendNode: Element) {
  ...
  private renderClipPath(clip: string) {
    this.el
      .select("defs")
      .append("clipPath")
