import { filter, find, forEach, get, groupBy, includes, isFinite, map, partition, reduce, sortBy } from "lodash/fp";
...
class DateFocus {
  ...
  private onMouseMove(mousePosition: MousePosition) {
    ...
    const timeAxisComputed = computedAxes.computed[timeAxis] as TimeAxisComputed;
    const orientation = timeAxis[0] as AxisOrientation;
  ...
  private clampDate(ticks: Array<Tick<Date>>) {
    return (date: Date) => {
  ...
  private focusDate(date: Date) {
    ...
    const mainAxisComputed = computedAxes.computed[mainAxis] as TimeAxisComputed;

    if (!includes(date.toString())(map((tick: Tick<Date>) => tick.value.toString())(mainAxisComputed.ticks))) {
      return;
    }

    const focusDates: DatesToFocus = {
      main: {
        date,
        axis: mainAxis,
      },
    ...
    if (comparisonAxis) {
      const comparisonDate = (computedAxes.computed[comparisonAxis] as TimeAxisComputed).scale.invert(
        mainAxisComputed.scale(date),
      );
  ...
  private focus(dates: DatesToFocus) {
    ...
    const position: number = Math.round(
      (this.state.current.getComputed().axes.computed[dates.main.axis as AxisPosition] as TimeAxisComputed).scale(
        dates.main.date,
      ),
    );
  ...
  private drawLine(isVertical: boolean, position: number) {
  ...
  private drawPoints(focusData: FocusDatum[], isVertical: boolean, position: number) {
    const pointData: FocusDatum[] = filter(
      (series: FocusDatum) => series.displayPoint && isFinite(series.valuePosition),
    )(focusData);
    this.elGroup
      .selectAll("circle")
      .data(pointData)
      ...
      .attr("fill", get("color"))
      .attr("r", 4.5)
      .attr("cx", d => (isVertical ? position : d.valuePosition))
      .attr("cy", d => (isVertical ? d.valuePosition : position));
  }

  private drawLabel(dates: DatesToFocus, focusData: FocusDatum[], isVertical: boolean, position: number) {
    ...
    const dataForAxis = (priority: "main" | "comparison") =>
      filter((d: FocusDatum) => d.axisPriority === priority)(focusData);
  ...
  private drawItemsForAxis(labels: D3Selection, data: any, date: Date, axis: AxisPosition) {
    const formatter = (this.state.current.getComputed().axes.computed[axis] as TimeAxisComputed).formatter;
  ...
  private addTitle(el: D3Selection, title: string) {
    el.append("xhtml:li")
      .attr("class", "title")
      .text(title)
      .append("span");
