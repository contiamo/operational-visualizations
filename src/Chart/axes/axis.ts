...
class Axis {
  ...
  public update(options: AxisOptions | ComputedAxisInput) {
  ...
  public draw(computed: AxisComputed, duration?: number) {
    this.computed = computed;
    this.ticks = this.options.hideAxis ? [] : (this.computed.ticks as Tick[]).filter(tick => !tick.hideTick);
    this.translateAxis();
  ...
  private drawTitle() {
    ...
    const title = this.el
      ...
      .data(this.options.title && !this.options.hideAxis ? [this.options.title] : []);
