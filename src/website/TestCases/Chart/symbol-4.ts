...
const renderer = (palette: string[]) => {
  return {
    type: "symbol",
    accessors: {
      fill: (_: any, d: any) => palette[Math.floor(d.sales / (2500000 / palette.length)) % palette.length],
      focusContent: (_: any, d: any) => [
      ...
      stroke: () => "transparent",
      symbol: (_: any, d: any) => (d.store === "MegaStore" ? "cross" : "diamond"),
      opacity: () => 0.4,
    },
  };
};

const createData = (palette: string[]): Data => {
  return {
    series: [
      {
