...
const createRenderer = (options: any) => {
  return {
    ...
    accessors: {
      ...
      stroke: options.stroke || ((series: any) => series.legendColor()),
...
const createData = (options: any): Data => {
  return {
    series: [
      {
        data: [
...
export const marathon = ({ test, afterAll, container }: MarathonEnvironment): void => {
  ...
  test("Change symbols", () => {
    viz.data(createData({ symbol: (_: any, d: any) => symbols[d.y] }));
    viz.draw();
  });
  ...
  test("Change sizes", () => {
    viz.data(
      createData({
        symbol: (_: any, d: any) => symbols[d.y],
        size: (_: any, d: any) => 25 * d.y,
      }),
    );
    viz.draw();
  });
  ...
  test("Change opacities", () => {
    viz.data(
      createData({
        ...
        stroke: (series: any, d: any) => (d.y % 2 === 0 ? series.legendColor() : "red"),
        fill: (series: any, d: any) => (d.y % 2 === 0 ? series.legendColor() : "red"),
        opacity: (_: any, d: any) => (d.y % 2 === 1 ? 0.4 : 0.6),
      }),
    );
    viz.draw();
  });
